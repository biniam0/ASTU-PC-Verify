...
export function HomePage() {
  ...
  const availableLaptops =
    data?.availableLaptops ?? MOCK_DASHBOARD.availableLaptops;
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 text-gray-600">
          Use the sidebar to register students and laptops, manage records, or
          run verification.
        </p>
      </div>
      ...
      {/* Today & alerts overview */}
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
          <h3 className="mb-2 text-sm font-semibold text-gray-900">
            Today at a glance
          </h3>
          <dl className="grid grid-cols-2 gap-3 text-sm text-gray-700 sm:grid-cols-3">
            <div>
              <dt className="text-gray-500">Today scans</dt>
              <dd className="text-base font-semibold text-gray-900">
                {loading ? "–" : (stats.todayScans ?? "0")}
              </dd>
            </div>
            <div>
              <dt className="text-gray-500">Students w/o laptop</dt>
              <dd className="text-base font-semibold text-gray-900">
                {loading ? "–" : (stats.studentsWithoutLaptops ?? "0")}
              </dd>
            </div>
            <div>
              <dt className="text-gray-500">New students today</dt>
              <dd className="text-base font-semibold text-gray-900">
                {loading ? "–" : (stats.todayStudentRegistrations ?? "0")}
              </dd>
            </div>
            <div>
              <dt className="text-gray-500">New laptops today</dt>
              <dd className="text-base font-semibold text-gray-900">
                {loading ? "–" : (stats.todayLaptopRegistrations ?? "0")}
              </dd>
            </div>
          </dl>
        </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
          ...
          {alertsSummary ? (
            <dl className="grid grid-cols-2 gap-3 text-sm text-gray-700">
              <div>
                <dt className="text-gray-500">Total alerts</dt>
                <dd className="text-base font-semibold text-gray-900">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
          ...
          {recentActivity.length === 0 ? (
            <p className="text-sm text-gray-500">No recent scans or alerts.</p>
          ) : (
              ...
              {recentActivity.slice(0, 5).map((item) => (
                ...
                >
                  <span className="truncate">
                    <span className="mr-1 rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium uppercase text-gray-700">
                      {item.kind === "alert" ? "Alert" : "Scan"}
                    </span>
                    {item.studentIdentifier ?? item.type ?? "—"}
                  </span>
