...
import { registerLaptop, uploadLaptopImage } from "@/services/laptopService";
import type { RegisterLaptopPayload } from "@/types/laptop";

// Must match backend MAC_REGEX and serial number validation
...
export function RegisterLaptopPage() {
  ...
  function updateField<K extends keyof RegisterLaptopPayload>(
    key: K,
    value: RegisterLaptopPayload[K],
  ) {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      setError(
        "Please fill in all required fields (Student ID, Brand Name, Model, Serial number).",
      );
      return;
    }
    if (!SERIAL_REGEX.test(serialNumber.trim())) {
      setError("Serial number can only contain letters, numbers, and hyphens.");
      return;
    }
    if (macAddress && macAddress.trim() && !MAC_REGEX.test(macAddress.trim())) {
      setError("Mac address must be in the format 00:1A:2B:3C:4D:5E.");
      return;
    }
    // Core backend registration only requires data fields; image upload is handled separately.
    if (!frontView || !backView || !serialNumberImage) {
      setError(
        "Please upload required images: Front view, Back view, and Serial number.",
      );
      return;
    }
    setIsLoading(true);
    try {
      const laptop = await registerLaptop(form);

      // After core laptop record is created, upload images to match backend expectations.
      const uploads: Promise<unknown>[] = [];
      if (frontView)
        uploads.push(uploadLaptopImage(laptop.id, "front", frontView));
      if (backView)
        uploads.push(uploadLaptopImage(laptop.id, "back", backView));
      if (serialNumberImage)
        uploads.push(uploadLaptopImage(laptop.id, "serial", serialNumberImage));
      if (macAddressScreen)
        uploads.push(uploadLaptopImage(laptop.id, "mac", macAddressScreen));
