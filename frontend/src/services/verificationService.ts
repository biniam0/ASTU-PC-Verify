...
export async function getVerificationStats(): Promise<VerificationStats> {
  const res = await apiRequest<{
    overall: { totalScans: number; registered: number; unregistered: number; noLaptops: number }
    today: { totalScans: number; registered: number; unregistered: number; noLaptops: number }
...
export async function getVerificationLogs(): Promise<VerificationLogEntry[]> {
  const res = await apiRequest<{ logs?: VerificationLogEntry[] }>('/logs/scans/today')
  const logs = res?.logs
...
export async function verifyByStudentId(studentId: string): Promise<VerificationResult> {
  const res = await apiRequest<BackendVerificationResult>('/verification/scan', {
    method: 'POST',
  ...
  const laptop = Array.isArray(res.laptops) && res.laptops.length > 0 ? res.laptops[0] : undefined
