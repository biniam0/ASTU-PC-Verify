...
export async function registerLaptop(payload: RegisterLaptopPayload): Promise<Laptop> {
  ...
  const res = await fetch(`${API_BASE}/students/${encodeURIComponent(studentId)}/laptops`, {
    method: 'POST',
    headers: {
  ...
  const data = (await res.json()) as { laptop: BackendLaptop; student: { full_name?: string } }
...
export async function updateLaptop(
  id: string,
  payload: Pick<RegisterLaptopPayload, 'brandName' | 'model' | 'serialNumber' | 'macAddress'>,
): Promise<Laptop> {
  const res = await fetch(`${API_BASE}/laptops/${encodeURIComponent(id)}`, {
    method: 'PUT',
    headers: {
...
export async function uploadLaptopImage(
  ...
  const res = await fetch(`${API_BASE}/laptops/${encodeURIComponent(laptopId)}/images`, {
    method: 'POST',
    headers: {
      ...getAuthHeader(),
