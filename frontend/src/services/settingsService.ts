...
export async function getDepartments(): Promise<Department[]> {
  const res = await apiRequest<{ departments?: Department[] }>(
...
export async function addDepartment(payload: {
  ...
  const res = await apiRequest<{ department: Department }>(
    "/settings/departments",
    {
      method: "POST",
...
export async function updateDepartment(
  ...
  const res = await apiRequest<{ department: Department }>(
    `/settings/departments/${encodeURIComponent(deptCode)}`,
    {
      method: "PUT",
      body: JSON.stringify(payload),
    },
...
export async function deleteDepartment(code: string): Promise<void> {
  await apiRequest<{ message: string }>(
    `/settings/departments/${encodeURIComponent(code)}`,
    {
      method: "DELETE",
    },
  );
...
export async function updateGate(
  ...
  const res = await apiRequest<{ gate: GateConfig }>(
    `/settings/gates/${encodeURIComponent(gateId)}`,
    {
      method: "PUT",
      body: JSON.stringify(payload),
    },
...
export async function getSecuritySettings(): Promise<SecuritySettingRow[]> {
  const res = await apiRequest<{ settings?: SecuritySettingRow[] }>(
    "/settings/security",
  );
...
export async function triggerManualBackup(): Promise<{
  ...
  const res = await apiRequest<{ backup: { id: string; status: string } }>(
