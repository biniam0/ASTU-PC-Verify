...
export async function fetchProfile(): Promise<AuthUser> {
  ...
  if (res?.user) {
    persistAuth({ token: window.localStorage.getItem('authToken') ?? '', user: res.user })
    return res.user
  }
...
export async function updateOwnProfile(userId: string, payload: UpdateProfilePayload): Promise<AuthUser> {
  const res = await apiRequest<{ user: AuthUser }>(`/users/${encodeURIComponent(userId)}`, {
    method: 'PUT',
...
export async function requestPasswordReset(email: string): Promise<{ message: string }> {
  throw new Error('Password reset via email is not implemented on the backend yet.')
}
